...
def extract_menu_name(line, price_text=None):
    ...
    # 오타 교정 적용
    menu_name = correct_typos(menu_name)
...
TYPO_CORRECTIONS = {
    ...
    "부대찌게": "부대찌개", "청국장찌게": "청국장찌개",
    # 볶음류
    "볶읍": "볶음", "뽁음": "볶음", "볶옴": "볶음", "복음": "볶음",
    "제육볶읍": "제육볶음", "오징어볶읍": "오징어볶음",
    "볶읍밥": "볶음밥", "복음밥": "볶음밥",
    # 국수/면류
    "칼국숫": "칼국수", "칼굿수": "칼국수", "칼국쑤": "칼국수",
    "짜장": "짜장", "자장면": "짜장면",
    "짬봉": "짬뽕", "짬뿡": "짬뽕",
    "냉면": "냉면", "랭면": "냉면",
    # 탕/국류
...
# 허용되는 영문 메뉴명 (한글 없어도 OK)
ENGLISH_FOOD_WORDS = [
    # 양식
    "pizza", "pasta", "steak", "burger", "hamburger", "salad", "soup",
    "risotto", "gnocchi", "lasagna", "spaghetti", "carbonara", "bolognese",
    # 일식
    "sushi", "sashimi", "ramen", "udon", "tempura", "teriyaki", "yakitori",
    "donburi", "katsu", "gyoza", "takoyaki",
    # 중식
    "dimsum", "dumpling", "wonton",
    # 음료/디저트
    "coffee", "latte", "espresso", "americano", "cappuccino", "mocha",
    "tea", "juice", "smoothie", "shake", "ade",
    "cake", "cookie", "brownie", "muffin", "waffle", "pancake",
    "ice cream", "gelato", "parfait", "tiramisu", "cheesecake",
    # 기타
    "chicken", "pork", "beef", "lamb", "fish", "shrimp", "lobster",
    "rice", "noodle", "bread", "toast", "sandwich", "wrap", "taco", "burrito",
    "wine", "beer", "cocktail", "whiskey", "vodka", "sake",
]
...
def is_valid_food(menu_name):
    ...
    # 영문 음식명인지 먼저 체크 (영문 메뉴 허용)
    for eng_food in ENGLISH_FOOD_WORDS:
        if eng_food in menu_lower:
            return True  # 영문 음식명이면 바로 허용

    # 한글이 하나도 없으면 제외 (영어/숫자만인데 음식명 아님)
